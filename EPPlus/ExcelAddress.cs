...
namespace OfficeOpenXml
{
    ...
    public class ExcelAddressBase : ExcelCellBase
    {
        ...
        private string GetAddress()
        {
            ...
            {
                adr.AppendFormat("[{0}]", _wb);
            }

            if (!string.IsNullOrEmpty(_ws))
            {
                adr.AppendFormat("'{0}'!", _ws);
            }

            if (IsName)
                adr.AppendFormat(GetAddress(_fromRow, _fromCol, _toRow, _toCol));
            else
                adr.AppendFormat(GetAddress(_fromRow, _fromCol, _toRow, _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed));
            return adr.ToString();
        }
        #endregion
        ...
        internal string FullAddress
        {
            get
            {
                ...
                if (_addresses != null)
                {
                    ...
                    {
                        a.AppendFormat(",{0}", sa.GetAddress());
                    }
                }
                else
                {
                    a = new StringBuilder(GetAddress());
                }
                return a.ToString();
            }
        }
